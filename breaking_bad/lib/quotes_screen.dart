...
class QuotesScreen extends StatelessWidget {
  ...
  Future<List<Quote>> fetchQuote(id) async => http
      .get(Uri.parse('https://breakingbadapi.com/api/characters/$id'))
      .then((charRes) => http
              .get(Uri.parse(
                  'https://breakingbadapi.com/api/quote?author=${jsonDecode(charRes.body)[0]['name'].split(' ').join('+')}'))
              .then((quoteRes) {
            // print(jsonDecode(quoteRes.body));
            List quotes = jsonDecode(quoteRes.body);
            return quotes.map((quote) => Quote.fromJson(quote)).toList();
          }).catchError(
                  (err) => throw Exception('Failed to load quotes from API')))
      .catchError(
          (err) => throw Exception('Failed to load characeter $id from API'));
